...
class TestNMTTransformer(unittest.TestCase):
  def setUp(self):
    ...
    self.train_transformer = transformer.NMTDataTransformer(nmtrain.enum.DataMode.TRAIN, 
                                                            vocab=vocab, 
                                                            data_analyzer=data_analyzer, 
                                                            unk_freq_threshold=1)
    for line in self.train_corpus:
      for word in line.split():
        data_analyzer.add_word_count(word)
  ...
  def test_transform_batch(self):
    ...
    numpy.testing.assert_array_equal(corpus[0], expected_batch)
...
class TestNMTTestTransformer(unittest.TestCase):
  def setUp(self):
    ...
    for line in train_data:
      for word in line.split():
        vocab.add_word(word)
    self.test_transformer = transformer.NMTDataTransformer(nmtrain.enum.DataMode.TEST,
                                                           vocab = vocab)
