...
class EncoderDecoderNMT(chainer.Chain):
  def __init__(self, in_size, out_size, hidden_units, drop_out, lstm_depth):
    ...
    self.add_link("encoder", encoders.BidirectionalEncoder(in_size, hidden_units, drop_out, lstm_depth))
    self.add_link("decoder", decoders.LSTMDecoder(out_size, hidden_units, drop_out, lstm_depth))
    self.train = False
